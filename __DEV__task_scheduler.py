# Uses the new COM Task Scheduler Interface to create a new disabled scheduled task, then run it once as part of a script. 
# Use this to launch interactive tasks, even remotely.
...
action_path = r'"C:\Programs\PowerScraper Stable Release\Experimental\PowerScraper.exe"' #executable path (could be python.exe)
action_arguments = r'username pw' #arguments (could be something.py)
action_workdir = r"" #working directory for action executable
author = "Ryan Xu, AAST 2020" #so that end users know who you are
description = "testing task from py" #so that end users can identify the task
task_id = "Test Task Updated"
